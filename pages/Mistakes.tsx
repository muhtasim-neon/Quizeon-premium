  ...
  return (
    <div className="space-y-6">
       <div className="flex justify-between items-center">
            <div>
                <h1 className="text-3xl font-bold text-white">Mistake Review</h1>
                <p className="text-slate-400">Focus on what needs improvement</p>
            </div>
            {mistakes.length > 0 && (
       ...
       {mistakes.length === 0 ? (
         <div className="text-center py-20 bg-white/5 rounded-2xl border border-dashed border-white/10">
            ...
            <p className="text-slate-400">Great job! Keep practicing to maintain your streak.</p>
         </div>
       ) : (
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {mistakes.map((record) => (
                <GlassCard key={record.itemId} className="flex justify-between items-center group">
                    <div>
                        <div className="text-xs text-red-400 font-bold mb-1">Failed {record.count} times</div>
                        <div className="text-2xl font-bold text-white">{record.item.ja}</div>
                        <div className="text-sm text-slate-400">{record.item.en} ({record.item.romaji})</div>
                    </div>
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                         <Button variant="secondary" className="p-2 h-auto" onClick={() => handleRemove(record.itemId)}>
                             <Trash2 size={16} />
                         </Button>
                    </div>
                </GlassCard>
